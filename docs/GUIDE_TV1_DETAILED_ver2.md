...
## TASK 4.1: Create fact_shipment Table DDL
...
**Output:**
- ✅ fact_shipment table created

---

## TASK 4.2: Create SSIS Package: Load_Fact_Shipment.dtsx

**Chi tiết từng bước:**

### Step 1: Mở package
1. Visual Studio → New SSIS Package → `Load_Fact_Shipment.dtsx`
2. Tab **Control Flow**
3. Canvas trống

### Step 2: Thêm 1 Data Flow Task
1. Kéo **Data Flow Task**
2. Rename → `DFT - Load fact_shipment (Incremental)`

### Step 3: Vào Data Flow Tab
1. Double-click DFT → Tab **Data Flow**
2. Canvas trống

### Step 4: Add OLE DB Source
1. Kéo **OLE DB Source** vào canvas
2. Double-click:
   - **Connection:** `ShopeeThailandDW_OLEDB`
   - **Data access mode:** `SQL command`
   - **SQLCommand:** (xem bước 4.3)
3. Click **OK**

### Step 5: Thêm Lookups cho dimension keys
1. Kéo **Lookup** × 5 (cho seller, shipment_method, date keys)
2. Chain: Source → Lookup_seller → Lookup_method → Lookup_dates

### Step 6: Thêm OLE DB Destination
1. Kéo **OLE DB Destination**
2. Nối từ Lookup chain
3. Double-click:
   - **Table:** `[gold].[fact_shipment]`
   - **Tab Mappings:** map tất cả columns
4. Click **OK**

**Output:**
- ✅ Load_Fact_Shipment.dtsx ready

---

## TASK 4.3: OLE DB Source Query (Incremental)
...
SELECT
    ...
    s.expected_date,
    s.actual_date,
    DATEDIFF(DAY, s.shipment_date, s.actual_date) AS days_to_deliver,
    s.is_on_time,
    s.tracking_number
FROM staging.stg_shipments s
INNER JOIN staging.stg_orders o ON s.order_id = o.order_id
WHERE NOT EXISTS (
    SELECT 1 FROM gold.fact_shipment f
    WHERE f.shipment_id = s.shipment_id
);
```

**🔴 KEY POINT:** `WHERE NOT EXISTS` → Chỉ lấy shipments CHƯA có trong fact table

---

## TASK 4.4-4.6: SQL Queries (3 insights)
...
### Query 4: Shipment Performance by Method & On-time Rate

```sql
SELECT
    ds.shipment_method_key,
    COUNT(*) AS total_shipments,
    SUM(CAST(fs.is_on_time AS INT)) AS on_time_count,
    CAST(SUM(CAST(fs.is_on_time AS INT)) * 100.0 / COUNT(*) AS DECIMAL(5,2)) AS on_time_rate,
    AVG(fs.days_to_deliver) AS avg_delivery_days
FROM gold.fact_shipment fs
INNER JOIN gold.dim_shipment ds ON fs.shipment_method_key = ds.shipment_key
GROUP BY ds.shipment_method_key, ds.shipment_method
ORDER BY on_time_rate DESC;
```

### Query 5: Shipment Volume by Seller

```sql
SELECT TOP 10
    ds.seller_name,
    dl.province,
    COUNT(DISTINCT fs.shipment_id) AS total_shipments,
    SUM(CAST(fs.is_on_time AS INT)) AS on_time_count,
    CAST(SUM(CAST(fs.is_on_time AS INT)) * 100.0 / COUNT(*) AS DECIMAL(5,2)) AS on_time_rate,
    AVG(fs.days_to_deliver) AS avg_delivery_days
FROM gold.fact_shipment fs
INNER JOIN gold.dim_seller ds ON fs.seller_key = ds.seller_key
...
- ✅ Query 4: Shipment performance by method
- ✅ Query 5: Shipment volume by seller
...
4. Run `Load_Fact_Shipment.dtsx` → All green (🆕)
...
- [ ] Load_Fact_Shipment.dtsx ✅ working (🆕 Incremental)
