...
# TUẦN 3: FACT_ORDER + FACT_SELLER_SALES (🔴 INCREMENTAL)

---

## TASK 3.8: Create fact_seller_sales Table DDL
...
-- Index for incremental load & performance
...
**Output:**
- ✅ fact_seller_sales table created

---

## TASK 3.9: Create SSIS Package: Load_Fact_Seller_Sales.dtsx

**Mục tiêu:** Xây dựng package load doanh số bán hàng theo seller & product

### Step 1: Setup Package
1. Visual Studio → New SSIS Package: `Load_Fact_Seller_Sales.dtsx`
2. Tab **Control Flow**

### Step 2: Add Data Flow Task
1. Kéo **Data Flow Task**
2. Rename → `DFT - Load fact_seller_sales (Incremental)`

---

## TASK 3.10-3.11: OLE DB Source Query (🔴 INCREMENTAL LOGIC)
...
2. Double-click:
   - **Connection:** `ShopeeThailandDW_OLEDB`
   - **Data access mode:** `SQL command`
   - **SQLCommand:** (xem bước 3.10)

### Step 3: SQLCommand (WHERE NOT EXISTS - Incremental)

```sql
SELECT
    CONVERT(VARCHAR(100), CONCAT(oi.seller_id, '_', oi.product_id, '_', FORMAT(CAST(o.order_date AS DATE), 'yyyyMMdd'))) AS seller_sales_id,
    ds.seller_key,
    dp.product_key,
    dd.date_key,
    COUNT(DISTINCT o.order_id) AS total_orders,
    SUM(ISNULL(oi.quantity, 1)) AS total_quantity_sold,
    SUM(oi.unit_price * ISNULL(oi.quantity, 1)) AS total_revenue,
    SUM(ISNULL(oi.discount_amount, 0)) AS total_discount_amount,
    SUM(ISNULL(oi.commission_amount, 0)) AS total_commission_amount,
    SUM(ISNULL(oi.maintenance_amount, 0)) AS total_maintenance_amount,
    SUM((oi.unit_price * ISNULL(oi.quantity, 1)) - ISNULL(oi.discount_amount, 0)) AS net_revenue,
    CAST(AVG(oi.unit_price * ISNULL(oi.quantity, 1)) AS DECIMAL(12,2)) AS avg_order_value,
...
WHERE NOT EXISTS (
    SELECT 1 FROM gold.fact_seller_sales fss
    WHERE fss.seller_sales_id = CONCAT(oi.seller_id, '_', oi.product_id, '_', FORMAT(CAST(o.order_date AS DATE), 'yyyyMMdd'))
)
GROUP BY oi.seller_id, oi.product_id, o.order_date, ds.seller_key, dp.product_key, dd.date_key;
```

**🔴 KEY POINT:** `WHERE NOT EXISTS` → **Chỉ lấy seller-product-date combinations CHƯA có**

### Step 4: Lookups (nếu cần verify FKs)
1. Kéo **Lookup** × 3 (nếu muốn verify seller/product/date lookup)
2. Hoặc: embed tất cả JOINs trong SQL command (đơn giản hơn)

### Step 5: OLE DB Destination
1. Kéo **OLE DB Destination**
2. Nối từ OLE DB Source
3. Double-click:
   - **Table:** `[gold].[fact_seller_sales]`
   - **Tab Mappings:** 12 columns (auto-map)
4. Click **OK**

---

## TASK 3.12-3.13: Test Fact_Seller_Sales (2 runs - Incremental Verify)

### Step 1: First Run
1. **F5** hoặc Execute Package
2. Verify:
   ```sql
   SELECT COUNT(*) FROM gold.fact_seller_sales;
   -- Expected Run 1: ~1K-5K rows (aggregated seller-product-date combos)
   ```
...
2. Verify:
   ```sql
   SELECT COUNT(*) FROM gold.fact_seller_sales;
   -- Expected Run 2: Still same rows (no new inserts)
   ```
...
```sql
SELECT TOP 5
    CONVERT(NVARCHAR(100), seller_key) AS seller_key,
    CONVERT(NVARCHAR(100), product_key) AS product_key,
    total_orders,
    total_quantity_sold,
    total_revenue,
    net_revenue,
    avg_order_value
FROM gold.fact_seller_sales
ORDER BY total_revenue DESC;
```

**Output:**
- ✅ fact_seller_sales loaded (~1K-5K rows)
- ✅ Incremental logic verified (run 2 = no new inserts)
- ✅ Aggregation verified (revenue = sum of order amounts)
...
## TASK 3.14-3.16: SQL Queries (Additional)

### Query 4: Top Sellers by Sales (from fact_seller_sales)

```sql
SELECT TOP 10
    ds.seller_name,
    dl.province,
    SUM(fss.total_orders) AS total_orders,
    SUM(fss.total_quantity_sold) AS total_qty,
    SUM(fss.total_revenue) AS total_revenue,
    SUM(fss.net_revenue) AS net_revenue,
    AVG(fss.avg_order_value) AS avg_order_value
FROM gold.fact_seller_sales fss
INNER JOIN gold.dim_seller ds ON fss.seller_key = ds.seller_key
INNER JOIN gold.dim_location dl ON ds.location_key = dl.location_key
WHERE ds.is_current = 1
GROUP BY ds.seller_name, dl.province
ORDER BY total_revenue DESC;
```

### Query 5: Top Products by Seller (from fact_seller_sales)
...
SELECT TOP 10
    ...
    dp.product_name,
    dpc.category_name,
    SUM(fss.total_orders) AS orders,
    SUM(fss.total_quantity_sold) AS qty_sold,
    SUM(fss.total_revenue) AS revenue,
    AVG(fss.avg_order_value) AS avg_order_value
FROM gold.fact_seller_sales fss
INNER JOIN gold.dim_seller ds ON fss.seller_key = ds.seller_key
INNER JOIN gold.dim_product dp ON fss.product_key = dp.product_key
INNER JOIN gold.dim_product_category dpc ON dp.category_key = dpc.category_key
WHERE ds.is_current = 1
GROUP BY ds.seller_name, dp.product_name, dpc.category_name
ORDER BY revenue DESC;
```

**Output:**
- ✅ Query 4: Top sellers by sales
- ✅ Query 5: Top products by seller
...
- [ ] fact_seller_sales: ~1K-5K rows (🆕 Incremental Aggregate)
- [ ] Incremental logic verified for both (run 2 = no new inserts)
...
- [ ] Load_Fact_Seller_Sales.dtsx ✅ working (🆕 Incremental)
